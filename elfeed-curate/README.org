...
* Content Publication
My preferred curation workflow is simple.
1. If an entry has *content of interest*, =star= it and add annotation if desired.
2. Publish all starred entries.
3. (optional) Tag all starred entries with a new date-based publication tag
   (e.g. + pub_DDMMYY). Then remove all of the star tags.

This is the elfeed search window showing starred entries to be exported. The entries with annotations (=ann=) are highlighted.
  [[screenshots/elfeed-curate-search-content.png]]

The exported content (as HTML) from the selected (starred) entries. This shows how entries are grouped and annotation examples.
