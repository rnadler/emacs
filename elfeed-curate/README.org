...
buffer for editing annotation content. Annotation can be added to an entry in both elfeed search and show mode. Most org-mode formatting will be exported
properly, but may differ depending on the export format.
...
4. The exported content is then displayed.
   * If the format is Markdown (=md=) and =elfeed-curate-hugo-base-dir= is
     ...
     (=elfeed-curate-hugo-section=). The Hugo development server should
